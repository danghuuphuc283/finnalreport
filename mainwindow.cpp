...
MainWindow::MainWindow(QWidget *parent)
    ...
    vertexCounter('A')
{
    onLoadFromFile("C:/learnspace/dirg/FINNAL/ford-bellman-final-main/graph.txt");
...
void MainWindow::onFindShortestPath() {
    ...
    // Xóa màu của các đỉnh và cạnh cũ, loại bỏ tất cả khỏi scene và bộ nhớ
    for (QGraphicsEllipseItem* vertexItem : coloredVertices) {
        vertexItem->setBrush(Qt::transparent);  // Xóa màu của các đỉnh
        scene->removeItem(vertexItem);          // Loại bỏ đỉnh khỏi scene
        delete vertexItem;                     // Xóa đối tượng khỏi bộ nhớ
    }

    // Xóa các cạnh cũ khỏi scene và khỏi danh sách coloredEdges
    for (QGraphicsLineItem* edgeItem : coloredEdges) {
        scene->removeItem(edgeItem);  // Loại bỏ các cạnh khỏi scene
        delete edgeItem;              // Xóa đối tượng khỏi bộ nhớ
    }

    // Xóa các item trong danh sách lưu trữ
    ...
    // Kiểm tra chu trình âm (Negative Cycle)
    for (const Edge& edge : edges) {
        QChar u = edge.from;
        QChar v = edge.to;
        int weight = edge.weight;
        ...
        if (distance[u] != INT_MAX && distance[u] + weight < distance[v]) {
            ...
            // Tính trọng số của chu trình âm
            int cycleWeight = 0;
            ...
            // Nếu chu trình âm, thông báo lỗi
            if (cycleWeight < 0) {
    ...
    // Kiểm tra và thông báo nếu phát hiện chu trình âm
    if (hasNegativeCycle) {
        QString cycleString;
        for (const QChar& vertex : negativeCycleVertices) {
            cycleString += QString(vertex) + " -> ";  // Chuyển QChar thành QString
        }

        if (!cycleString.isEmpty()) {
            cycleString.chop(4);  // Loại bỏ " -> " cuối cùng
        }
    ...
    // Tiến hành tính toán đường đi ngắn nhất nếu không có chu trình âm
    QList<QChar> path;
    ...
    for (int i = 0; i < path.size(); ++i) {
        ...
        vertexInfo.ellipseItem->setBrush(Qt::green);  // Đổi màu các đỉnh trên đường đi ngắn nhất
    ...
    for (int i = 0; i < path.size() - 1; ++i) {
        ...
        line->setPen(QPen(Qt::red, 3));  // Đổi màu các cạnh trên đường đi ngắn nhất
        scene->addItem(line);
...
int MainWindow::calculateCycleWeight(const QList<QChar>& cycle) {
    ...
    // Duyệt qua các cặp đỉnh liên tiếp trong chu trình để tính trọng số
    for (int i = 0; i < cycle.size() - 1; ++i) {
        QChar u = cycle[i];
        QChar v = cycle[i + 1];

        // Tìm cạnh nối u và v trong danh sách các cạnh
        for (const Edge& edge : edges) {
            if (edge.from == u && edge.to == v) {
                cycleWeight += edge.weight;
                break;
